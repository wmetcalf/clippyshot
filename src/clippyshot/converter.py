            ...
            if scan_png is None:
                ...
                try:
                    if ocr_helper is not None and ocr_helper.is_ready():
                        ...
                        try:
                            # An absolute DEADLINE, not a duration. WarmOCR
                            # serialises every caller on one lock, and a
                            # duration computed here is still the full duration
                            # when the lock is finally acquired -- so one 60s
                            # job budget would stretch across every queued page.
                            # An instant does not decay while waiting; the
                            # helper prices the call against it after the wait.
                            warm_deadline = (
                                time.monotonic() + ocr_time_left()
                                if ocr_time_left is not None
                                else None
                            )
                            result = ocr_helper.ocr(
                                scan_png, lang=ocr_lang, psm=ocr_psm,
                                timeout_s=max(1, per_call_timeout),
