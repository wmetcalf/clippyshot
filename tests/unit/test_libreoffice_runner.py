...
class FakeSandbox:
    ...
    def run(self, request: SandboxRequest) -> SandboxResult:
        ...
        stage_in = next(
            m.host_path for m in request.ro_mounts if m.sandbox_path == Path("/sandbox/in")
        )
...
def test_runner_falls_back_to_original_extension_when_rewrite_cannot_be_loaded(tmp_path: Path):
